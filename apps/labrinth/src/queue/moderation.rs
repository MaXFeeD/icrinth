...
impl ModerationMessages {
    ...
    pub fn markdown(&self, auto_mod: bool) -> String {
        ...
        if auto_mod {
            str.push_str("<hr />\n\n");
            str.push_str("🤖 This is an automated message generated by AutoMod (BETA). If you are facing issues, please [contact support](mailto:support@inner-core.org).");
        }
...
impl ModerationMessage {
    ...
    pub fn body(&self) -> String {
        match self {
            ...
            ModerationMessage::PackFilesNotAllowed { files, .. } => {
                let mut str = "".to_string();
                str.push_str("This pack redistributes copyrighted material. Please refer to [Modrinth guide on obtaining modpack permissions](https://support.modrinth.com/en/articles/8797527-obtaining-modpack-permissions) for more information.\n\n");
                ...
                fn print_mods(projects: Vec<&String>, headline: &str, val: &mut String) {
                    ...
                    for project in &projects {
                        ...
                        val.push_str(&if let Some(additional_text) = additional_text {
                            format!("- {project}(consider using [{additional_text}](https://inner-core.org/mod/{}) instead)\n", additional_text.to_lowercase())
                        } else {
                            format!("- {project}\n")
                        })
                    }
            ...
            ModerationMessage::MissingGalleryImage => "We ask that resource packs like yours show off their content using images in the Gallery, or optionally in the Description, in order to effectively and clearly inform users of the content in your pack per section 2.1 of [Inner Core Mods content rules](https://inner-core.org/legal/rules#general-expectations).\n
