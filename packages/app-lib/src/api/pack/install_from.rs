...
pub enum PackDependency {
    #[serde(rename = "innercore")]
    InnerCore,

    #[serde(rename = "coreengine")]
    CoreEngine,
...
pub async fn set_profile_information(
    ...
    for (key, value) in dependencies {
        match key {
            PackDependency::InnerCore => {
                mod_loader = Some(ModLoader::InnerCore);
                loader_version = Some(value);
            }
            PackDependency::CoreEngine => {
                mod_loader = Some(ModLoader::CoreEngine);
                loader_version = Some(value);
            }
