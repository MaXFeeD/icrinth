...
pub async fn create_mrpack_json(
    ...
    match (profile.loader, profile.loader_version.clone()) {
        (crate::prelude::ModLoader::InnerCore, Some(v)) => {
            dependencies.insert(PackDependency::InnerCore, v)
        }
        (crate::prelude::ModLoader::CoreEngine, Some(v)) => {
            dependencies.insert(PackDependency::CoreEngine, v)
        }
